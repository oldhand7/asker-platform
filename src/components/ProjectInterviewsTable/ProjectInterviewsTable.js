...
const getColumns = ({ handleAction, compare = [],  project, sort, order, t, onEditCandidate, onConductInterview, anonimize = false }) => ([
  {
    title: <Link href={getSortLink('candidate.name', sort, order, project)}>
      <a>{t('labels.candidate')} {getSortArrowIcon('candidate.name', sort, order)}</a>
    </Link>,
    dataIndex: 'name',
  ...
  },
  {
    title: <Link href={getSortLink('score', sort, order, project)}>
      <a>{t('headings.total-interview-score')} {getSortArrowIcon('score', sort, order)}</a>
    </Link>,
    dataIndex: 'score',
    key: 'score',
    render: (score) => {
  ...
  },
  {
    title: <Link href={getSortLink('updatedAt', sort, order, project)}>
      <a>{t('headings.date-interview')} {getSortArrowIcon('updatedAt', sort, order)}</a>
    </Link>,
    dataIndex: 'updatedAt',
    render: (updatedAt, row) => {
      if (row.status == 'awaiting') {
        return <PlatformButton className={styles['project-interviews-table-start-button']} onClick={e => {
          e.stopPropagation();
          onConductInterview(row)
        }}>
          <PlayIcon /> {t('actions.start-interview')}</PlatformButton>
      }
  ...
  },
  {
    title: <a href={`/projects/${project.id}/overview/`}>
      {t('labels.action-plural')} <FilterIcon />
    </a>,
    render: (_, row) => {
      return <div className={styles['project-interviews-table-actions']}>

        {row.status != 'awaiting' ? <>
        <Tooltip text={t('actions.candidate-compare')}>{setRef => (
          <CompareButton active={compare.indexOf(row) > -1} ref={setRef} onClick={e => handleAction('compare', row, e)} />)}
        </Tooltip>
        <Tooltip text={t('actions.edit-response')}>{setRef => (
          <EditButton ref={setRef} onClick={e => onConductInterview(row)} />)}
        </Tooltip></> : null}
        <Tooltip text={t('actions.delete')}>{setRef => (
          <TrashButton title="" ref={setRef} onClick={e => handleAction('delete', row, e)} />)}</Tooltip>
      </div>
    }
  }
]);

const ProjectInterviewsTable = ({ className, data = [], onEditCandidate, onDelete, onConductInterview, compare, onCompare, project, ...props }) => {

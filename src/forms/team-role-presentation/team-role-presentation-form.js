...
import styles from './team-role-presentation-form.module.scss';
...
const TeamRolePresentationForm = ({ values, className, onValues, onError }) => {
  ...
  return <div className={classNames(className, styles['team-role-presentation-form'])}>
    <TextareaInputField error={errors && errors['notes']} className={classNames(styles['team-role-presentation-form-field'], styles['team-role-presentation-form-notes'])} name="notes" onChange={control.input('notes')} placeholder="Enter your introductory text" />
...

    {
      ...
      <FileDropInputField maxFiles={Math.max(BUNDLE_MAX_FILES - formValues.files.length, 0)} bundleReminingBytes={bundleReminingBytes} error={errors && errors['files']} label="Upload file" onFiles={handleFiles} multiple={false} className={styles['team-role-presentation-form-field']} /> :
      <p className={styles['team-role-presentation-form-warning']}>
